      ...
      {items.map((it, i) => (
        <details key={i} id={`${idPrefix}-${i}`} className="card p-5 md:p-6">
          <summary className="cursor-pointer list-none flex items-start justify-between gap-6">
            <span className="font-display text-lg text-navy-950">{it.q}</span>
            <span className="chev mt-1 text-gold-600" aria-hidden="true">
  ...
  return (
    <details className="card p-5 md:p-6">
      <summary className="cursor-pointer list-none flex items-center justify-between gap-4">
        <span className="font-display text-xl text-navy-950">{title}</span>
        <span className="text-xs uppercase tracking-widest text-gold-600 flex items-center gap-1">
          {cta}
